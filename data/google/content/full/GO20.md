...
![](https://martinfowler.com/articles/pushing-ai-autonomy/card.png)

_We ran a series of experiments to explore how far Generative AI can currently be pushed toward_
_autonomously developing high-quality, up-to-date software without human intervention._
_As a test case, we created an agentic workflow to build a simple Spring Boot application end to end._
_We found that the workflow could ultimately generate these simple applications,_
_but still observed significant issues in the results—especially as we increased the complexity._
_The model would generate features we hadn't asked for,_
_make shifting assumptions around gaps in the requirements,_
_and declare success even when tests were failing._
_We concluded that while many of our strategies — such as reusable prompts or a reference application —_
_are valuable for enhancing AI-assisted workflows, a human in the loop to supervise generation remains essential._
...
[generative AI](https://martinfowler.com/tags/generative%20AI.html)

## Contents

- [The strategies](https://martinfowler.com/articles/pushing-ai-autonomy.html#TheStrategies)
  - [Choice of the tech stack](https://martinfowler.com/articles/pushing-ai-autonomy.html#ChoiceOfTheTechStack)
  - [Multiple agents](https://martinfowler.com/articles/pushing-ai-autonomy.html#MultipleAgents)
  - [Stack-specific over general purpose](https://martinfowler.com/articles/pushing-ai-autonomy.html#Stack-specificOverGeneralPurpose)
  - [Use of deterministic scripts](https://martinfowler.com/articles/pushing-ai-autonomy.html#UseOfDeterministicScripts)
  - [Code examples in prompts](https://martinfowler.com/articles/pushing-ai-autonomy.html#CodeExamplesInPrompts)
  - [Reference application as an anchor](https://martinfowler.com/articles/pushing-ai-autonomy.html#ReferenceApplicationAsAnAnchor)
  - [Generate-review loops](https://martinfowler.com/articles/pushing-ai-autonomy.html#Generate-reviewLoops)
  - [Codebase modularization](https://martinfowler.com/articles/pushing-ai-autonomy.html#CodebaseModularization)
- [Results](https://martinfowler.com/articles/pushing-ai-autonomy.html#Results)
  - [Round 1: 3-5 entities](https://martinfowler.com/articles/pushing-ai-autonomy.html#Round13-5Entities)
  - [Round 2: Pre-existing schema with 10 entities](https://martinfowler.com/articles/pushing-ai-autonomy.html#Round2Pre-existingSchemaWith10Entities)
  - [A game of whac-a-mole](https://martinfowler.com/articles/pushing-ai-autonomy.html#AGameOfWhac-a-mole)
  - [Overeagerness](https://martinfowler.com/articles/pushing-ai-autonomy.html#Overeagerness)
  - [Gaps in the requirements will be filled with assumptions](https://martinfowler.com/articles/pushing-ai-autonomy.html#GapsInTheRequirementsWillBeFilledWithAssumptions)
  - [Brute force fixes](https://martinfowler.com/articles/pushing-ai-autonomy.html#BruteForceFixes)
  - [Declaring success in spite of red tests](https://martinfowler.com/articles/pushing-ai-autonomy.html#DeclaringSuccessInSpiteOfRedTests)
  - [Static code analysis issues](https://martinfowler.com/articles/pushing-ai-autonomy.html#StaticCodeAnalysisIssues)
- [Summary of the experiment](https://martinfowler.com/articles/pushing-ai-autonomy.html#SummaryOfTheExperiment)
- [What you can take away for your augmented workflows](https://martinfowler.com/articles/pushing-ai-autonomy.html#WhatYouCanTakeAwayForYourAugmentedWorkflows)
- [About the workflow development process](https://martinfowler.com/articles/pushing-ai-autonomy.html#AboutTheWorkflowDevelopmentProcess)
- [Final reflections](https://martinfowler.com/articles/pushing-ai-autonomy.html#FinalReflections)
  - [Can future LLMs resolve these issues?](https://martinfowler.com/articles/pushing-ai-autonomy.html#CanFutureLlmsResolveTheseIssues)
  - [Questions that remain](https://martinfowler.com/articles/pushing-ai-autonomy.html#QuestionsThatRemain)

### Sidebars

- [Subtasking versus TODO lists:](https://martinfowler.com/articles/pushing-ai-autonomy.html#SubtaskingVersusTodoLists)
- [What about parallelisation?](https://martinfowler.com/articles/pushing-ai-autonomy.html#WhatAboutParallelisation)
...
_simple_ application, today. The standard and the quality lens applied to
...
![](https://martinfowler.com/articles/pushing-ai-autonomy/target_application-widened.png)
...
- A _common tech stack_ that should be quite prevalent in the training
data
- A runtime _framework that can do a lot of the heavy lifting_, which means
less code to generate for AI
- An application topology that has very clearly _established patterns_:
...
![](https://martinfowler.com/articles/pushing-ai-autonomy/agent-workflow.png)
...
![](https://martinfowler.com/articles/pushing-ai-autonomy/subtasking-setup.png)
...
language specifications into code in _any_ stack. However, just telling
...
have to become to make the results high quality _and_ repeatable.
...
![](https://martinfowler.com/articles/pushing-ai-autonomy/reference-application.png)
...
In an LLM’s first generation, it often doesn’t follow _all_ of the
...
![](https://martinfowler.com/articles/pushing-ai-autonomy/sample-package-structure-widened.png)
...
maintainability and extensibility - for both humans _and_ AI:
...
intervention. It _always_ ran into some errors, but could frequently fix its
own errors itself.
...
| 3\. Follows instructions | Our prompts and code samples included a few specific instructions for certain libraries<br> and coding patterns we wanted, to test fidelity of the generated code to standards prescribed in the prompts | Achieved most of the time, but slightly less reliable than the "Well-tested" criterion |
| 4\. Static code analysis | No serious static code analysis issues | Debatable because "seriousness" is debatable, but for an application that needs long-term<br> maintainability, some of the issues still remaining were<br> problematic. |
| 5\. Functionality | As intended, and not more than we asked for | Inconsistent. This got especially concerning when we increased the complexity to 10<br> entities and tried some change requests, we saw AI make assumptions<br> about business logic, and delete an important validation. Overall, we<br> found that it was a bit like a game of "whac-a-mole", every time we<br> ran the workflow, something else happened. |
...
We set out to learn more about how far we could push the _autonomy_ of
...
learnings away from this to enhance _augmented_ workflows.
...
this to try out some “reference drift detection”, where we asked the agent to
...
With that, let's move on to the conclusions about AI coding _autonomy_:
...
don't want. [Kent Beck famously calls coding agents “genies”](https://tidyfirst.substack.com/p/augmented-coding-and-design), because they give
you what you wish for but in their own way, exploiting all the loopholes in
the human phrasing of the wish. And there is a _very_ high variety of things that can
...
Instead of hoping for “better models”, what if augmentation indeed will remain a fact,
...
I was reminded of this question when Ethan Mollick recently wrote about the “Bitter Lesson”:
...
like [Claude Flow](https://github.com/ruvnet/claude-flow) are trying to do, creating a “swarm”
of agents that work together to solve a problem, with access to lots of tools and memory strategies.
This “brute force” approach is probably appropriate for a bunch of use cases, and something worth monitoring.
...
Thanks to the Thoughtworks “AI-assisted software” hivemind for many discussions and inspirations, in particular
Beto Frega, Brandon Cook, Folker Bernitt, James Lewis, Julien Deswaef, Kief Morris, Moritz Wilke, Vanya Seth, Zhenjia Zhou,
for providing ideas or reviewing and discussing this content.

Significant Revisions

_05 August 2025:_ published